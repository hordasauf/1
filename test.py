...
def calculate_trend_change_indicator(df, params):
    ...
    # Extract parameters
    indicator_name = params.get('trend_change_indicator1', 'ker_5m')
    upper_threshold = params.get('trend_change_upper_threshold', 0.6)
    lower_threshold = params.get('trend_change_lower_threshold', 0.6)

    # =========================================================
    # MODIFIED SECTION WITH CUSTOM INDICATOR FORMULA
    # =========================================================

    if indicator_name in df.columns:
        # Check if indicator was above upper_threshold in previous period
        # and below lower_threshold in current period
        condition = (df[indicator_name].shift(1) > upper_threshold) & (df[indicator_name] < lower_threshold)

        # Set signal to 1 where condition is met
        result[condition] = 1

        # You could add debugging information if needed
        signal_count = condition.sum()
        print(f"Generated {signal_count} signals for condition: {indicator_name} drop from >{upper_threshold} to <{lower_threshold}")
...
def optimize_binary_signal_prediction(data_directory, output_directory, parameter_space=None,
    ...
    iteration_counter = [0]  # Use a list to allow modification inside nested functions
    """
    ...
    if SKOPT_AVAILABLE:
        # Define a callback to track iterations
        def iteration_callback(res):
    ...
    if train_period is None:
        ...
        train_end = datetime(2024, 10, 10)
    else:
        train_start, train_end = train_period

    if test_period is None:
        test_start = datetime(2024, 10, 11)
    ...
    def objective(params_list):
        ...
        print(f"Avg Price Change: {evaluation['avg_price_change']:.6f}")
        print(f"Avg Future SMA Change: {evaluation['avg_future_sma_change']:.6f}")
        print(f"Estimated time remaining: {iteration_time * (n_calls - optimizer_iteration):.2f} seconds")
        print("-" * 50)
...
def main():
    ...
    parameter_space = {
        ...
        'trend_change_upper_threshold': (0.6, 0.62),  # Range for upper threshold
        'trend_change_lower_threshold': (0.57, 0.6),  # Range for lower threshold

        # Future prediction parameters (how far ahead to predict)
        'future_period': [3, 6, 12],  # Look 15min, 30min, 60min ahead
    ...
    result = optimize_binary_signal_prediction(
        ...
        n_calls=20,  # Number of optimization iterations (increase for better results)
